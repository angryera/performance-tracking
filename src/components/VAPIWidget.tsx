...
import { AlertCircle, Clock, Mic, MicOff, Phone, Play, Square, Volume2, VolumeX } from 'lucide-react'
import { useEffect, useRef, useState } from 'react'
import { ConnectingOverlay, ProcessingOverlay } from './components/Overlays'
import { ErrorToast } from './components/Toast'
import { Instructions } from './components/Instructions'
import { AnamVideoWidget } from './components/AnamVideoWidget'
import { formatTimestamp, getMessageAge } from '@/lib/utils'
...
export default function VAPIWidget({
  ...
  // Helper function to scroll message list to bottom
  ...
  const getAllMessages = () => {
    ...
    console.log('🕐 Sorted messages by timestamp:', sortedMessages.map(msg => ({
      id: msg.id,
      role: msg.role,
      timestamp: msg.timestamp,
      age: getMessageAge(msg.timestamp)
    })))
  ...
  return (
    <>
      ...
      {process.env.NODE_ENV === 'development' && showAnamChat && (
        <div className="right-6 bottom-6 !z-100 fixed">
          <div className="bg-black bg-opacity-70 shadow-2xl backdrop-blur-sm p-4 border border-gray-600 rounded-2xl max-w-sm">
            <div className="flex justify-between items-center mb-3 font-medium text-white text-sm">
            ...
            <div className="space-y-2 text-gray-300 text-xs">
              <div className="gap-3 grid grid-cols-2">
                <div className="bg-gray-800 bg-opacity-50 px-2 py-1 rounded">
                  ...
                  <div className="font-medium">{userMessages.length}</div>
                </div>
                <div className="bg-gray-800 bg-opacity-50 px-2 py-1 rounded">
                  <div className="text-gray-400 text-xs">Anam</div>
                  <div className="font-medium">{conversationHistory.length}</div>
                </div>
                <div className="col-span-2 bg-blue-800 bg-opacity-50 px-2 py-1 rounded">
                  <div className="text-gray-400 text-xs">Total</div>
                  <div className="font-medium">{getAllMessages().length}</div>
                </div>
              </div>
              <div className="bg-gray-800 bg-opacity-50 px-2 py-1 rounded">
              ...
              <div className="bg-gray-800 bg-opacity-50 px-2 py-1 rounded">
                <div className="text-gray-400 text-xs">Session Status</div>
                <div className="font-medium text-green-400">Active</div>
              </div>
              <div className="bg-gray-800 bg-opacity-50 px-2 py-1 rounded">
                <div className="text-gray-400 text-xs">Assistant</div>
                <div className={`font-medium ${isAssistantSpeaking ? 'text-yellow-400' : 'text-gray-400'}`}>
                  {isAssistantSpeaking ? 'Speaking...' : 'Silent'}
                </div>
              </div>
              {/* Timestamp Debug Info */}
              {getAllMessages().length > 0 && (
                <div className="col-span-2 bg-gray-800 bg-opacity-50 px-2 py-1 rounded">
                  <div className="text-gray-400 text-xs">Timestamp Debug</div>
                  <div className="font-medium text-xs">
                    Latest: {getMessageAge(getAllMessages()[getAllMessages().length - 1]?.timestamp || '')}
                  </div>
                  <div className="text-gray-400 text-xs">
                    Raw: {getAllMessages()[getAllMessages().length - 1]?.timestamp || 'None'}
                  </div>
                </div>
              )}
            </div>
